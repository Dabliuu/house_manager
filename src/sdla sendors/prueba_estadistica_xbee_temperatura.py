...
class Xbee_sensor_manager():
    ...
    def get_xbee_data(
        ...
        print(">> Temperatura: "+str(float_vrms))

        float_irms = struct.unpack('>f', separated[1]) 
        print(">> Humedad: "+str(float_irms))
...
if __name__ == "__main__":
    ...
    try:
        ...
        #REMOTE_NODE_ID = "WFM" # watter flow measurement
        # REQUEST_PDU = bytearray([87, 70]) # request pdu for instant "WF" Volumen y flujo

        REMOTE_NODE_ID = "FE" # watter flow measurement
        REQUEST_PDU = bytearray([84, 72]) # request pdu for instant "TH" Temperatura y humedad
